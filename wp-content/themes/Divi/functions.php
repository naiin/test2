...
//My hooks and actions for mycred Integration with gravity forms Quizes

add_filter("gform_pre_submission", "wdm_evaluate_results");

function wdm_evaluate_results($form)
{
	$total = 0;
	foreach($form['fields'][0]['choices'] as $values){
		$total += (int) $values['gquizWeight'];
	}
	$_POST['points_total'] = $total;
}
...
function Learndash_myCRED_Hook( $installed ) {
	...
	$installed['hook_learndash'] = array(
		'title'       => __( 'QuizHook', 'mycred' ),
		'description' => __( 'Rewards %_plural% for quiz submission actions', 'mycred' ),
		'callback'    => array( 'myCRED_Hook_Rewardquiz' )
	);
...
if ( ! class_exists( 'myCRED_Hook_Rewardquiz' ) && class_exists( 'myCRED_Hook' ) ) {
	class myCRED_Hook_Rewardquiz extends myCRED_Hook {
		...
		public function run() {
			...
			if ( $this->prefs['course_completed']['creds'] != 0 )
				add_action( 'gform_after_submission',    array( $this, 'course_completed' ), 5, 1 );
		...
		public function course_completed( $data ) {
			...
			if ( $this->core->has_entry( 'course_completed', $data['id'], $user_id ) ) return;
			//Check if quiz is passed or not
			$points = 0;
			if ($data['gquiz_is_pass'] == 1) $points = ((int)$this->prefs['course_completed']['creds']) + (int)$_POST['points_total'] ;
			else $points = $this->prefs['course_completed']['creds'];
			print_r($data);
			print $points;
			// Execute
			$this->core->add_creds(
		...
		public function add_references( $references ) {
			...
			$references['course_completed'] = 'Completed Quiz';
			return $references;
		...
		public function preferences() {
            ...
            <label class="subheader" for="<?php echo $this->field_id( array( 'course_completed' => 'creds' ) ); ?>"><?php _e( 'Completing a Quiz', 'mycred' ); ?></label>
            <ol>
                <li>
...
//end of custom mycred Integration by me.
